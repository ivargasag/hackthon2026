...
def generate_tests(text_changes):
    ...
    for change in text_changes:
        ...
        # Test 1: El nuevo texto existe en la página
        test_cases.append(f"""
  test('should display updated text "{new_text_val}"', async ({{ page }}) => {{
    await page.goto(BASE_URL);
    const el = page.locator('{selector}');
    await expect(el).toBeVisible();
    await expect(el).toHaveText('{new_text_val}');
  }});""")
        
        # Test 2: El texto viejo ya no debe existir
        if old_text_val:
            test_cases.append(f"""
  test('should NOT display old text "{old_text_val}"', async ({{ page }}) => {{
    await page.goto(BASE_URL);
    const el = page.locator('{selector}');
